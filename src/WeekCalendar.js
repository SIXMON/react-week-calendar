...
class WeekCalendar extends React.Component {
    ...
    handleSelectionStop = e => {
        ...
        const { firstDay, scaleUnit, useModal } = this.props;
        const {
            startSelectionPosition,
            mousePosition,
            scaleIntervals,
        ...
        const startSelectionTime = Utils.getMoment(
            scaleUnit,
            minCellIndex,
            offsetTop
        );
        const endSelectionTime = Utils.getMoment(
            scaleUnit,
            maxCellIndex,
            offsetTop
        );
    ...
    submitPreselectedInterval = newValue => {
        ...
        if (updateEvent) {
            ...
            const intervals = Utils.getIntervals(
        ...
        for (let dayIndex = 0; dayIndex < numberOfDays; dayIndex += 1) {
            ...
            if (intervals.length > 0) {
                ...
                intervals.forEach((interval, index, array) => {
                    ...
                    const newHeight =
                        l * cellHeight + (l % 2 == 0 ? l - 2 : l - 1);

                    const height = l > 2 ? newHeight : l * cellHeight;
                    ...
                    const eventComponent = (
                        ...
                        >
                            {React.cloneElement(this.props.eventComponent, {
                                ...interval,
                            })}
                        </div>
                    );
    ...
    renderOverlay() {
        if (this.state.startSelectionPosition != null) {
            ...
            const pos = Math.min(startPosition.y, mousePosition.y);
            const top =
                pos * this.props.cellHeight + (pos % 2 == 0 ? pos : pos - 1);

            let { left } = this.state.columnDimensions[
            ...
            const height =
                ...
                top +
                (Math.max(startPosition.y, mousePosition.y) % 2 == 0
                    ? Math.max(startPosition.y, mousePosition.y) - 2
                    : Math.max(startPosition.y, mousePosition.y) - 1);
            ...
            return (
                <div className="calendarModal">
                    ...
                    <div className="calendarModal__content">
                        {React.cloneElement(this.props.modalComponent, {
                            ...preselectedInterval,
